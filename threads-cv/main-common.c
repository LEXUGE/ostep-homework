...
void usage() {
  ...
  fprintf(stderr, "  -P <sleep string: how each producer should sleep at "
                  "various points in execution>\n");
  fprintf(stderr, "  -C <sleep string: how each consumer should sleep at "
                  "various points in execution>\n");
  fprintf(stderr,
...
int main(int argc, char *argv[]) {
  ...
  if (producer_pause_string != NULL)
    parse_pause_string(producer_pause_string, "producers", producers,
                       producer_pause_times);
  if (consumer_pause_string != NULL)
    parse_pause_string(consumer_pause_string, "consumers", consumers,
                       consumer_pause_times);
